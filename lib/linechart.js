...
function resetData(config, timesteps) {
    console.log ("Resetting data");

    for (t = 0; t < timesteps; t++) {
        config.data.datasets[0].data[t] = ({x:t, y:0});
        config.data.datasets[1].data[t] = ({x:t, y:0});
        config.data.datasets[2].data[t] = ({x:t, y:0});
        config.data.datasets[3].data[t] = ({x:t, y:0});
    }
}