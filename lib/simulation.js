...
class Actor {  
    ...
    tick(config){
        if (this.status === ACTOR_STATUS.EXPOSED) {
            ...
            if(this.infectedTime > config.daysToSymptoms && !this.isAsymptomatic) {
...
class Simulation {
    ...
    constructor(simulationParameters) {
        ...
        for (let cnt = 0; cnt < Math.max(1, this.config.startingInfectionRate * this.config.populationSize); cnt++){
        ...
        for (const actor of this.actors) {
            if (actor.status === ACTOR_STATUS.INFECTIOUS && !actor.isolated) { 
                ...
                for (let encounter = 0; encounter < this.config.numInteractions; encounter++){
                    ...
                    this.checkExposure(others, actor);
                }
            }
        } 
    }
