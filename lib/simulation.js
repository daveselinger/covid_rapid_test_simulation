...
class SimulationParameters {
    ...
    /** Testing rate PCR -- approximately 1 MM/day */
    testingRatePcr = 0.003;
...
/**
 * Use a bates distribution to approximate a gaussian
 * @param {*} mean 
 * @param {*} std 
 */
function gaussianRandom(mean, std=1) {
    ...
    rand /= 6;
    return Math.max(0,mean + (rand-0.5) * std);
}
class Infection {
    ...
    */
    ...
    constructor(actor) {
        ...
        this.daysToSymptomatic = gaussianRandom(params.daysToSymptoms);
        this.daysToNotSymptomatic = this.daysToSymptomatic + gaussianRandom(params.daysToRecovery);
        this.daysToPcrDetectable = gaussianRandom(params.daysToPcrDetectable);
        this.daysToPcrNotDetectable = this.daysToPcrDetectable + gaussianRandom(params.durationDaysOfPcrDetection);
        /* this assures that the sampled daysToAntigenDetectible is after the sampled daysToPcrDetectible */
        this.daysToAntigenDetectable = this.daysToPcrDetectable + gaussianRandom(params.daysToAntigenDetectable-params.daysToPcrDetectable);
        this.daysToAntigenNotDetectable =this.daysToAntigenDetectable +  gaussianRandom(params.durationDaysOfAntigenDetection);
    }
...
//        this.testTime = Math.floor(Math.random() * this.simulationParameters.testingInterval);
    ...
    tick(days=1.0){
        ...
        // TODO: These calls to random seem incorrect based on the approach.
        if (this.status === ACTOR_STATUS.EXPOSED) {
...
class Simulation {
    ...
    tickPcrTesting(days = 1.0) { 
        // Perform pcr testing
        for (const actor of this.actors) {
            if(actor.isTestingPcr 
            && (actor.testTimePcr === null || actor.testTimePcr >= this.simulationParameters.testingIntervalPcr)
            && !actor.isolated
                && actor.pcrTest()
            ) {
                // TODO: Can sample these as well.
